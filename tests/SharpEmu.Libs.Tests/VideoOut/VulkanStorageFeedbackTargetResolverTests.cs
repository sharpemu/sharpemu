...
public sealed class VulkanStorageFeedbackTargetResolverTests
{
    ...
    public void ZeroWriteSingleTargetAliasUsesCompatibilityAttachment()
    {
        ...
        var supported = VulkanStorageFeedbackTargetResolver.TryResolve(
            [target], [Texture(TargetAddress, true)], [Blend(0)],
            out var resolved, out var usedCompatibilityAttachment, out var unsupportedAddress);
    ...
    {
        var supported = VulkanStorageFeedbackTargetResolver.TryResolve(
            [Target()], [Texture(TargetAddress, true)], [Blend(0xF)],
            out var resolved, out var usedCompatibilityAttachment, out var unsupportedAddress);
    ...
    [Fact]
    public void NonStorageAliasDoesNotNeedCompatibilityAttachment()
    {
        var supported = VulkanStorageFeedbackTargetResolver.TryResolve(
            [Target()], [Texture(TargetAddress, false)], [Blend(0xF)],
            out var resolved, out var usedCompatibilityAttachment, out var unsupportedAddress);
    ...
    [Fact]
    public void AliasedMrtRemainsUnsupported()
    {
        const ulong otherAddress = 0x1042_F00000;
        var supported = VulkanStorageFeedbackTargetResolver.TryResolve(
            [Target(), Target(otherAddress)],
            [Texture(TargetAddress, true)], [Blend(0), Blend(0)],
            out _, out var usedCompatibilityAttachment, out var unsupportedAddress);
    ...
    private static GuestDrawTexture Texture(ulong address, bool isStorage) =>
        new(address, 2048, 2048, Format: 10, NumberType: 0, RgbaPixels: [],
            IsFallback: false, IsStorage: isStorage);
