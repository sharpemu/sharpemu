...
public static partial class KernelMemoryCompatExports
{
    ...
    {
        if (string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_WIDE"), "1", StringComparison.Ordinal))
        {
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_WIDE_PRINTF"), "1", StringComparison.Ordinal))
        {
            return;
        }
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_WIDE_PRINTF_ARGS"), "1", StringComparison.Ordinal))
        {
            return false;
        }

        var filter = Environment.GetEnvironmentVariable("HYPER5_LOG_WIDE_PRINTF_FILTER");
    ...
    private static void TraceLibcAllocation(
        ...
        int? errorCode = null)
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_LIBC_ALLOC"), "1", StringComparison.Ordinal))
        {
            return;
        }
    ...
    private static string? ResolveApp0Root()
    {
        ...
        var configured = Environment.GetEnvironmentVariable("HYPER5_APP0_DIR");
    ...
    {
        var configuredRoot = Environment.GetEnvironmentVariable("HYPER5_DEVLOG_APP_DIR");
        string root;
    ...
    {
        const string temp0VariableName = "HYPER5_TEMP0_DIR";
        ...
        var app0Root = Environment.GetEnvironmentVariable("HYPER5_APP0_DIR");
        ...
        var root = Path.Combine(Path.GetTempPath(), "Hyper5", appName, "temp0");
    ...
    private static string ResolveDownload0Root()
    {
        ...
        const string download0VariableName = "HYPER5_DOWNLOAD0_DIR";
    ...
    {
        const string hostappVariableName = "HYPER5_HOSTAPP_DIR";
    ...
    private static string GetPerAppWritableRoot()
    {
        ...
        var invalidChars = Path.GetInvalidFileNameChars();
        appName = new string(appName.Select(ch => invalidChars.Contains(ch) ? '_' : ch).ToArray());
        return Path.Combine(Path.GetTempPath(), "Hyper5", appName);
    }
    ...
    {
        return string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_DIRECT_MEMORY"), "1", StringComparison.Ordinal);
    }
    ...
    private static unsafe bool TryAllocateLibcHeapCore(nuint requestedSize, nuint alignment, bool zeroFill, out ulong address)
    {
        ...
        var guardHeap = string.Equals(
            Environment.GetEnvironmentVariable("HYPER5_GUARD_HEAP"),
            "1",
            StringComparison.Ordinal);
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_OPEN"), "1", StringComparison.Ordinal))
        {
            return;
        }
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("HYPER5_LOG_IO"), "1", StringComparison.Ordinal))
        {
            return;
        }

        var filter = Environment.GetEnvironmentVariable("HYPER5_LOG_IO_FILTER");
